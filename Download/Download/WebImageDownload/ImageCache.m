...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns diskCacheDirectory:(nonnull NSString *)directory {
    if ((self = [super init])) {
        NSString *fullNamespace = [@"com.hackemist.ImageCache." stringByAppendingString:ns];
        // 创建 IO 串行队列
        _ioQueue = dispatch_queue_create("com.hackemist.ImageCache", DISPATCH_QUEUE_SERIAL);
...
- (void)storeImage:(nullable UIImage *)image
    ...
    if (toDisk) {
        dispatch_async(self.ioQueue, ^{
            ...
            if (!data && image) {
                ImageFormat imageFormatFromData = [NSData imageFormatForImageData:data];
                data = [image imageDataAsFormat:imageFormatFromData];
            }
...
- (void)diskImageExistsWithKey:(nullable NSString *)key completion:(nullable CheckCacheCompletionBlock)completionBlock {
...
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key done:(nullable CacheQueryCompletedBlock)doneBlock {
    if (!key) {
        if (doneBlock) {
            doneBlock(nil, nil, ImageCacheTypeNone);
        }
        return nil;
    }
    UIImage *image = [self imageFromMemoryCacheForKey:key];
    ...
    dispatch_async(self.ioQueue, ^{
        ...
        @autoreleasepool {
            ...
            if (doneBlock) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    doneBlock(diskImage, diskData, ImageCacheTypeDisk);
                });
            }
        }
    });
